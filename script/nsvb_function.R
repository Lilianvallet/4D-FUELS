nsvb <- function(tree){
  library(tidyverse)
  setwd(paste0(projectP,"/data/Biomass_data/"))

# Species reference table
ref_spp <- read_csv("REF_SPECIES.csv",show_col_types = FALSE) |> 
...
get_hm <- function(top_dia, a, DIA, b, HT, c, alpha, beta) {
  ...
  result <- uniroot(equation, interval = c(1, HT), extendInt = "yes")
...
vol_ib <- read_csv("Table S1a_volib_coefs_spcd.csv",show_col_types = FALSE) |> 
  rename(MODEL = model)
vol_ib_jenkins <- read_csv("Table S1b_volib_coefs_jenkins.csv",show_col_types = FALSE) |> 
  rename(MODEL = model)
...
vol_bark <- read_csv("Table S2a_volbk_coefs_spcd.csv",show_col_types = FALSE) |> 
  rename(MODEL = model)
vol_bark_jenkins <- read_csv("Table S2b_volbk_coefs_jenkins.csv",show_col_types = FALSE) |> 
  rename(MODEL = model)
...
vol_ob1 <- read_csv("Table S3a_volob_coefs_spcd.csv",show_col_types = FALSE) |> 
  rename(MODEL_ob = model)
vol_ob2 <- read_csv("Table S4a_rcumob_coefs_spcd.csv",show_col_types = FALSE) |> 
  rename(MODEL_rcumob = model)
vol_ob <- inner_join(vol_ob1, vol_ob2, by = c("SPCD", "DIVISION", "STDORGCD"))

vol_ob1_jenkins <- read_csv("Table S3b_volob_coefs_jenkins.csv",show_col_types = FALSE) |> 
  rename(MODEL_ob = model)
vol_ob2_jenkins <- read_csv("Table S4b_rcumob_coefs_jenkins.csv",show_col_types = FALSE) |> 
  rename(MODEL_rcumob = model)
...
vol_ratio <- read_csv("Table S5a_rcumib_coefs_spcd.csv",show_col_types = FALSE) |> 
  rename(MODEL = model)
vol_ratio_jenkins <- read_csv("Table S5b_rcumib_coefs_jenkins.csv",show_col_types = FALSE) |> 
  rename(MODEL = model)
...
wdsg <- read_csv("REF_SPECIES.csv",show_col_types = FALSE) |> 
...
bark_wt <- read_csv("Table S6a_bark_biomass_coefs_spcd.csv",show_col_types = FALSE) |> 
  rename(MODEL = model)
bark_wt_jenkins <- read_csv("Table S6b_bark_biomass_coefs_jenkins.csv",show_col_types = FALSE) |> 
  rename(MODEL = model) |> 
  mutate(b1 = NA)
...
SPCD_levels <- levels(as.factor(tree$SPCD))
DIVISION_levels <- levels(as.factor(tree$DIVISION))
mat_levels <- expand.grid(SPCD_levels,DIVISION_levels)
names(mat_levels) <- c("SPCD","DIVISION")
TEST <- NULL
for(i in 1:nrow(mat_levels)){
  TEST[i]<- length(which(spp_bark_wt_all$SPCD==mat_levels$SPCD[i]&spp_bark_wt_all$DIVISION==mat_levels$DIVISION[i]))
}

if(any(TEST<1)){
  missing_row <- which(TEST!=1)
  for(j in missing_row){
    spp_bark_wt_all%>%
      filter(SPCD==mat_levels$SPCD[j])->tab
    spp_bark_wt_all%>%
      ungroup%>%
      add_row(DIVISION=mat_levels$DIVISION[j],
              SPCD=tab$SPCD[1],
              STDORGCD=tab$STDORGCD[1],
              MODEL=tab$MODEL[1],
              a=tab$a[1],
              b=tab$b[1],
              b1=tab$b1[1],
              c=tab$c[1],
              coef_level=tab$coef_level[1])->spp_bark_wt_all
  }
}
...
branch_wt <- read_csv("Table S7a_branch_biomass_coefs_spcd.csv",show_col_types = FALSE) |> 
  rename(MODEL = model)
branch_wt_jenkins <- read_csv("Table S7b_branch_biomass_coefs_jenkins.csv",show_col_types = FALSE) |> 
  rename(MODEL = model) |> 
  mutate(b1 = NA)
...
TEST <- NULL
for(i in 1:nrow(mat_levels)){
  TEST[i]<- length(which(spp_branch_wt_all$SPCD==mat_levels$SPCD[i]&spp_branch_wt_all$DIVISION==mat_levels$DIVISION[i]))
}
  ...
  for(j in missing_row){
    ...
    spp_branch_wt_all%>%
      ...
      add_row(DIVISION=mat_levels$DIVISION[j],
              ...
              b1=tab$b1[1],
              c=tab$c[1],
              coef_level=tab$coef_level[1],
              WDSG=tab$WDSG[1])->spp_branch_wt_all
  }
}
...
cr_prop <- read_csv("Table S11_mean_crprop.csv",show_col_types = FALSE) |> 
...
eco_div_prov <- read_csv("eco_div_prov.csv",show_col_types = FALSE) |> 
...
agb <- read_csv("Table S8a_total_biomass_coefs_spcd.csv",show_col_types = FALSE) |> 
  rename(MODEL = model)
agb_jenkins <- read_csv("Table S8b_total_biomass_coefs_jenkins.csv",show_col_types = FALSE) |> 
  rename(MODEL = model)
...
TEST <- NULL
for(i in 1:nrow(mat_levels)){
  TEST[i]<- length(which(spp_agb_all$SPCD==mat_levels$SPCD[i]&spp_agb_all$DIVISION==mat_levels$DIVISION[i]))
}
...
foliage <- read_csv("Table S9a_foliage_coefs_spcd.csv",show_col_types = FALSE) |> 
  rename(MODEL = model)
foliage_jenkins <- read_csv("Table S9b_foliage_coefs_jenkins.csv",show_col_types = FALSE) |> 
...
TEST <- NULL
for(i in 1:nrow(mat_levels)){
  TEST[i]<- length(which(spp_foliage_all$SPCD==mat_levels$SPCD[i]&spp_foliage_all$DIVISION==mat_levels$DIVISION[i]))
}
  ...
  for(j in missing_row){
    ...
    spp_foliage_all%>%
      ...
      add_row(DIVISION=mat_levels$DIVISION[j],
              ...
              c=tab$c[1],
              c1=tab$c1[1],
              coef_level=tab$coef_level[1])->spp_foliage_all
  }
}
...
# Carbon fractions
spp_live <- read_csv("Table S10a_fia_wood_c_frac_live.csv",show_col_types = FALSE) |> 
  select(SPCD, fia.wood.c) |> 
  rename(CF = fia.wood.c) |> 
  filter(SPCD <= 999)

spp_dead <- read_csv("Table S10b_fia_wood_c_frac_dead.csv",show_col_types = FALSE) |> 
